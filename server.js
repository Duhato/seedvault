...
app.get('/api/backup/export-zip', authMiddleware, async (req, res) => {
  try {
    const [species, varieties, seedLots, plants, projects, harvest, germination, amendments, crosses, observations, locations, sources, weatherLog, frostEvents, companions] = await Promise.all([
      ...
      pool.query('SELECT * FROM weather_log ORDER BY log_date'),
...
app.post('/api/backup/import-zip', authMiddleware, async (req, res) => {
  ...
  uploadZip.single('backup')(req, res, async (err) => {
    ...
    try {
      ...
      for (const w of (data.weather_logs || [])) { await client.query('INSERT INTO weather_log (log_date, source, condition, high_temp_f, low_temp_f, humidity_pct, precip_inches, wind_mph, notes) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) ON CONFLICT (log_date) DO NOTHING', [w.log_date, w.source, w.condition, w.high_temp_f, w.low_temp_f, w.humidity_pct, w.precip_inches, w.wind_mph, w.notes]); }
